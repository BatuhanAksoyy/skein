    ...
    def execute(self, context):
        ...
        try:
            ...
            if rpc_response is not None and "error" in rpc_response:
                ...
                self.report(
                    {"ERROR"},
                    "request for Bevy registry data returned an error, is the Bevy Remote Protocol Plugin added and is the Bevy app running? :: "
                    + brp_response["error"]["message"],
                )
                return {"CANCELLED"}
            print("\nexecute: TODO: c")
            ...
            self.report(
                {"ERROR"},
                "Could not connect to bevy application to fetch registry data from the Bevy Remote Protocol",
            )
            return {"CANCELLED"}
...
class ReloadSkeinRegistryJson(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if "skein-registry.json" in bpy.data.texts:
            embedded_registry = json.loads(
                bpy.data.texts["skein-registry.json"].as_string()
            )
...
def process_registry(context, registry):
    ...
    fake_component_enum_annotations = {
        ...
        "selected_type_path": bpy.props.StringProperty(
            name="Selected Type Path", default="Unknown"
        ),
    }
    ...
    for type_path, value in registry.items():
        ...
        try:
            property_group_or_property = make_property(
                skein_property_groups, registry, type_path
            )
            if "reflectTypes" in value and "Component" in value["reflectTypes"]:
                ...
                if inspect.get_annotations(property_group_or_property):
                    fake_component_enum_annotations[maybe_hashed_type_path] = (
